...
export function ProductDetailsDialog({ product }) {
  ...
  const handelAddToCart = async () => {
    try {
      ...
      if (!selectedSize) return showToast("error", "please select size");
      await dispatch(
        addToCart({
          userEmail,
          product,
  ...
  useEffect(() => {
    if (selectedSize) {
      const firstColorForSize = product.sizes.find(
  ...
  const allProductsColors = [
    ...new Map(
      product?.sizes?.flatMap((s) => s.colors).map((c) => [c.colorName, c]),
    ).values(),
  ];

  // colors depend on selected size
  const currentColors = selectedSize
    ? product.sizes.find((s) => s.id === selectedSize.id)?.colors || []
    : [];
  return (
    <Dialog className={"overflow-y-scroll"}>
      <form>
        ...
        <DialogContent className="max-w-[90vw] md:max-w-[700px] lg:max-w-[1000px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className={"text-center text-accent text-xs font-bold"}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
              ...
              <div className="md:col-span-7 lg:col-span-8">
                <div className="flex flex-col gap-3.5 text-[16px]">
                  ...
                  <DialogDescription
                    className={"text-accent text-sm leading-[1.6]"}
                  >
                    {product.description}
                  </DialogDescription>
                  <Size
                  ...
                  <Color
                    ...
                    selectedSize={selectedSize}
                    setSelectedMainImg={setSelectedMainImg}
                  ...
                  <div className="flex items-center justify-start gap-4 mt-5">
                    ...
                    <button
                      ...
                      className="bg-primary cursor-pointer h-[45px] w-[230px] text-white text-xs font-medium transition-colors duration-200 hover:bg-secondary"
                    >
                      ADD TO CART
                    </button>
                  </div>
